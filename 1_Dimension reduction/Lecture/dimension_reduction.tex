...
$\rightharpoondown$ \alert{Noise free formulation} of this problem, i.e. $X = AS$, was proposed in the signal processing litterature\footnote{{\tiny Blind separation of sources, part I: an adaptive algorithm based on neuromimetic architecture. {\bf Jutten, C. et al.}, {\em Signal Processing, 2(4), 1-10}}, 1991}
\end{frame}
...
$\rightharpoondown$ The  components $S_i$ \alert{are not Gaussian random variables} (with the possible exception of one component).

\vspace{.4cm}

$\rightharpoondown$ $d\geqslant n$, i.e. \alert{the number of oservations is greater than the number of independent components}.

\vspace{.4cm}

$\rightharpoondown$ The matrix $A$ has \alert{full rank}.
\end{frame}

\begin{frame}{Linear independent component analysis - identifiability}
Assume that $d\geqslant n$, and that the components $S_i$ \alert{are not Gaussian random variables} (with the possible exception of one component) in the model

\begin{align*}
\Acolorboxed{& X = AS\eqsp.}
\end{align*}


\vspace{.4cm}

Assume also that the probability density $p_S$ of $S$ belogns to a subset $\mathsf{E}$ of probability densities on $\rset^n$ which admit (at least) a moment up to order $2$.

\vspace{.4cm}

Then, if \alert{$X= \tilde A \tilde S$}, where the probability density $p_{\tilde S}$ of $\tilde S$ lies in $\mathsf{E}$ and if \alert{the contrast function $\Psi$ is discriminant over $\mathsf{E}$}, $\Psi(p_{\tilde S}) = \Psi(p_{S})$ {\bf \textcolor{violet}{if and only if $\tilde A = A \Lambda P$ where $\Lambda$ is an invertible diagonal matrix and $P$ is a permutation matrix}}.
...
$\rightharpoondown$  \textcolor{lighto}{Nonlinear independent component analysis} (ICA)\footnote{{\tiny Independent component analysis: algorithms and applications, {\bf Hyvarinen, A. et al.}, {\em Neural networks, 13(4-5), 411-430}}, 2000}.
