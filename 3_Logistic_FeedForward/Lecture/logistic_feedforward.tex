...
\begin{frame}{How to estimate the conditional law of $Y$?}
...
The \alert{conditional densities are modeled as multivariate normal}. For all class $k\in\{0,1\}$, conditionnally on $\{Y=k\}$,
\begin{align*}
X\sim \mathcal{N}(\mu_k, \Sigma_k)\,.
\end{align*}
\textcolor{violet}{Discriminant functions}:
$$
\psi_k: x\mapsto \ln (g_k(x) + \ln(\Prob{Y=k})
\eqsp.
$$
...
\begin{align*}
\Acolorboxed{&
f^*:x\mapsto \mathds{1}\{\psi_1(x)>\psi_{0}(x)\}\eqsp.}
\end{align*}
...
$\rightharpoondown$ How to relax the Gaussian assumption ? (\alert{logistic model}).

$\rightharpoondown$ How to design nonlinear classification rules ? (\alert{neural networks}).
